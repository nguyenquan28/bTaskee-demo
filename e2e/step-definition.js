...
const postTask = async (serviceId, address, description = 'My Task') => {
  ...
  if (address === '500 Thanon Tanao') {
    ...
    // }else{
    idAddress = '500 Thanon Tanao, แขวง วัดบวรนิเวศ เขตพระนคร กรุงเทพมหานคร ประเทศไทย'
    // }
  } else {
    await element(by.id('txtInputAddress')).typeText('12 ' + address);
    idAddress = '12 Đường Phạm Văn Nghị, Tân Phong, Quận 7, Thành phố Hồ Chí Minh, Việt Nam';
    if (address === 'Kim Ma, Ha Noi') {
      idAddress = '12 Kim Mã, Ba Đình, Hà Nội, Việt Nam';
    } else if (address === 'Bach Dang, Da Nang') {
      idAddress = '12 Bạch Đằng, Thạch Thang, Q. Hải Châu, Đà Nẵng, Việt Nam';
    }
  }
...
const chooseMarketAddress = async (address, description = 'My Task') => {
  ...
  if (address === '500 Thanon Tanao') {
    ...
    await element(by.id('txtInputAddress')).typeText('12 ' + address);
    idAddress = 'Chợ Tân Mỹ, Đường Tân Mỹ, Tân Phú, Quận 7, Thành phố Hồ Chí Minh, Việt Nam';
    if (address === 'Kim Ma, Ha Noi') {
      ...
      idAddress = '12 Đồng Khởi, Bến Nghé, Quận 1, Thành phố Hồ Chí Minh, Việt Nam'
...
const scroll = async (id, pixels, direction, startPositionX = NaN, startPositionY = NaN) => { // direction: 'up' or 'down'
  await element(by.id(id)).scroll(pixels, direction, startPositionX, startPositionY);
};
...
const selectTime24h = async (hour, hourDefault = 14) => {
  if (device.getPlatform() === 'ios') {
    ...
    if (hourTap > 0) {  // Tap the next time
      for (var i = 0; i < hourTap; i++) {
        await dp.tapAtPoint({ x: 110, y: 135 });
      }
    } else {  // Tap the previous time
      for (var i = 0; i > hourTap; i--) {
...
const fillActiveCode = async (phone, countryCode) => {
  const data = await initData('user/getActivationCode', { phone: phone, countryCode: countryCode });
...
const cancelTask = async ({ fee = '0', currency = '₫', promotion = false }) => {
  ...
  await tapText('Đồng ý');
  if (promotion) {
    await waitForElement('Nếu bạn hủy công việc có tính phí, bạn sẽ không sử dụng được mã khuyến mãi nữa.', 500, 'text');
    await tapText('Tiếp tục');
  }
  await tapText('Không cần công việc này nữa.');
  await tapText('Đồng ý');
  await waitForElement(`Công việc đã được hủy. Phí: ${fee} ${currency}.`, 500, 'text');
  await tapText('Đóng');
};

const houseKeepingPTCreateRoom = async (roomName = 'Full House', area = '50') => {
  // CREATE ROOM
  ...
  await waitForElement('Tạo buồng phòng thành công. Bạn có thể đăng việc ngay bây giờ!', 500, 'text');
...
const signUpWithModal = async (name = 'name', phone = '0834567890', email = 'test@demo.com', countryCode) => {
  try {
    await waitForElement('Select service', 2000, 'text');
    await tapId('btnNextIntroPage');
    await waitForElement('Task description', 500, 'text');
...
const enterInput = async (inputID, typeInput) => {
  const input = element(by.id(inputID))
  await input.typeText(typeInput)
}
